<template>
    ...
    <CKFormModal v-model="formModel" :title="params.id ? 'Edit Patient' : 'New Patient'" :endPoint="endPoint"
        :id="id" :params="params"
...
const title = ref("Patient Master List");
const endPoint = ref("/visits/patient");
const params = ref({});
...
const columns = ref([
    ...
    { accessorKey: 'full_name', header: 'Patient Name' },
    { id: 'action' }
]);
const loadData = async () => {
    ...
    try {
        ...
        if (temp.success && Array.isArray(temp.patients)) {
            data.value = temp.patients;
        } else {
...
const filteredData = computed(() => {
    ...
    return data.value.filter(patient => {
        ...
        const phoneMobile = patient.phone_mobile?.toLowerCase() || '';
        const uhidOld = patient.uhid_old?.toLowerCase() || '';
        return fullName.includes(query) || phoneMobile.includes(query) || uhidOld.includes(query);
    });
});
