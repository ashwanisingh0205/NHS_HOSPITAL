<template>
    ...
    <CKFormModal v-model="formModel" :title="params.id ? 'Edit Payer Mapping' : 'New Payer Mapping'"
        :endPoint="endPoint" :formCode="'item_payer_mapping'" :initialData="initialData" :params="params"
...
const title = ref("Payer Mapping List");
const endPoint = ref("/masters/items/payer_mapping");
const params = ref({});
...
const columns = ref([
    ...
    { accessorKey: 'payer_name', header: 'Payer Name' },
    { id: 'action' }
]);
const loadData = async () => {
    ...
    try {
        ...
        error.value = err.response?.data?.message || err.message || 'Failed to load payer mappings';
    } finally {
        loading.value = false;
    }
};
