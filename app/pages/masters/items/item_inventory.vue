<template>
    ...
    <CKFormModal v-model="formModel" :title="params.id ? 'Edit Item Inventory' : 'New Item Inventory'"
        :endPoint="endPoint" :formCode="'item_inventory_master'" :initialData="initialData" :params="params"
...
const title = ref("Item Inventory List");
const endPoint = ref("/masters/items/inventory");
const params = ref({});
...
const loadData = async () => {
    ...
    try {
        ...
        error.value = err.response?.data?.message || err.message || 'Failed to load item inventory items';
    } finally {
        loading.value = false;
    }
};
