<template>
    ...
    <CKFormModal v-model="formModel" :title="params.id ? 'Edit Item Hospital' : 'New Item Hospital'"
        :endPoint="endPoint" :formCode="'item_hospital_master'" :initialData="initialData" :params="params"
...
const title = ref("Item Hospital List");
const endPoint = ref("/masters/items/hospital");
const params = ref({});
...
const loadData = async () => {
    ...
    try {
        ...
        error.value = err.response?.data?.message || err.message || 'Failed to load item hospitals';
    } finally {
        loading.value = false;
    }
};
