<template>
    ...
    <CKFormModal v-model="formModel" :title="params.id ? 'Edit Item POS' : 'New Item POS'" :endPoint="endPoint"
        :formCode="'item_pos_master'" :initialData="initialData" :params="params"
...
const title = ref("Item POS List");
const endPoint = ref("/masters/items/pos");
const params = ref({});
...
const loadData = async () => {
    ...
    try {
        ...
        error.value = err.response?.data?.message || err.message || 'Failed to load item pos items';
    } finally {
        loading.value = false;
    }
};
