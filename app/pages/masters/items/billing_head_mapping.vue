        ...
        <div class="md:w-1/3 lg:w-1/3">
            <CKCardList :loading="loading" :title="title" :show-add="true" @handleAdd="handleAdd" v-model="searchQuery">
                <UTable :loading="loading" :data="filteredData" :columns="columns">
                    ...
                    <template #billing_head_name-cell="{ row }">
...
const title = ref("Billing Head Mapping List");
const endPoint = ref("/masters/items/billing_head");
const params = ref({});
...
const loadData = async () => {
    ...
    try {
        ...
        error.value = err.response?.data?.message || err.message || 'Failed to load billing heads';
    } finally {
        loading.value = false;
    }
};
...
const filteredData = computed(() => {
    ...
    return data.value.filter(item =>
        item.billing_head_name?.toLowerCase().includes(query)
    );
});
