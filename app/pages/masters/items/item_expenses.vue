<template>
    ...
    <CKFormModal v-model="formModel" :title="params.id ? 'Edit Item Expenses' : 'New Item Expenses'"
        :endPoint="endPoint" :formCode="'item_expenses_master'" :initialData="initialData" :params="params"
...
const title = ref("Item Expenses List");
const endPoint = ref("/masters/items/expenses");
const params = ref({});
...
const loadData = async () => {
    ...
    try {
        ...
        error.value = err.response?.data?.message || err.message || 'Failed to load item expenses';
    } finally {
        loading.value = false;
    }
};
