<template>
    ...
    <CKFormModal v-model="formModel" :title="params.id ? 'Edit Payer Master' : 'New Payer Master'" :endPoint="endPoint"
        :formCode="'item_payer_master'" :initialData="initialData" :params="params"
...
const title = ref("Payer Master List");
const endPoint = ref("/masters/items/payer_master");
const params = ref({});
...
const loadData = async () => {
    ...
    try {
        ...
        error.value = err.response?.data?.message || err.message || 'Failed to load payers';
    } finally {
        loading.value = false;
    }
};
