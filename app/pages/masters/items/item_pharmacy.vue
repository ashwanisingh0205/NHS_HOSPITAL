<template>
    ...
    <CKFormModal v-model="formModel" :title="params.id ? 'Edit Item Pharmacy' : 'New Item Pharmacy'"
        :endPoint="endPoint" :formCode="'item_pharmacy_master'" :initialData="initialData" :params="params"
...
const title = ref("Item Pharmacy List");
const endPoint = ref("/masters/items/pharmacy");
const params = ref({});
...
const loadData = async () => {
    ...
    try {
        ...
        error.value = err.response?.data?.message || err.message || 'Failed to load item pharmacy items';
    } finally {
        loading.value = false;
    }
};
