<template>
        ...
        <div class="md:w-1/3 lg:w-1/3">
            <CKCardList :loading="loading" :title="title" :show-add="true" @handleAdd="handleAdd" v-model="searchQuery">
                <UTable :loading="loading" :data="filteredData" :columns="columns">
                    ...
                    <template #corporate_name-cell="{ row }">
    ...
    <CKFormModal v-model="formModel" :title="params.id ? 'Edit Corporate' : 'New Corporate'" :endPoint="endPoint"
        :id="id" :params="params"
...
const title = ref("Unit Corporate Master List");
const endPoint = ref("/masters/units/corporate");
const params = ref({});
...
const columns = ref([
    ...
    { accessorKey: 'corporate_name', header: 'Corporate Name' },
    { id: 'action' }
]);
const loadData = async () => {
    ...
    try {
        ...
        if (temp.success && Array.isArray(temp.corporates)) {
            data.value = temp.corporates;
        } else {
            error.value = 'Invalid response format from API';
        }
    } catch (err) {
        error.value = err.response?.data?.message || err.message || 'Failed to load corporates';
    } finally {
        loading.value = false;
    }
};
...
const filteredData = computed(() => {
    ...
    return data.value.filter(corporate =>
        corporate.corporate_name?.toLowerCase().includes(query)
    );
});
