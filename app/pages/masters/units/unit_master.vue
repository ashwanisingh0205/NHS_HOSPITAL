<template>
    ...
    <CKFormModal v-model="formModel" :title="params.id ? 'Edit Unit' : 'New Unit'" :endPoint="endPoint"
        :id="id" :params="params"
...
const title = ref("Unit Master List");
const endPoint = ref("/masters/units/unit");
const params = ref({});
...
const loadData = async () => {
    ...
    try {
        ...
        if (temp.success && Array.isArray(temp.units)) {
            data.value = temp.units;
        } else {
            error.value = 'Invalid response format from API';
        }
    } catch (err) {
        error.value = err.response?.data?.message || err.message || 'Failed to load units';
    } finally {
        loading.value = false;
    }
};
