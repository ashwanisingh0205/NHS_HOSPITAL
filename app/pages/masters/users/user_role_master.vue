<template>
    ...
    <CKFormModal v-model="formModel" :title="params.id ? 'Edit User Role' : 'New User Role'" :endPoint="endPoint"
        :id="id" :params="params"
...
const title = ref("User Role Master List");
const endPoint = ref("/masters/users/role");
const params = ref({});
...
const loadData = async () => {
    ...
    try {
        ...
        if (temp.success && Array.isArray(temp.role)) {
            data.value = temp.role;
        } else {
            error.value = 'Invalid response format from API';
        }
    } catch (err) {
        error.value = err.response?.data?.message || err.message || 'Failed to load roles';
    } finally {
        loading.value = false;
    }
};
