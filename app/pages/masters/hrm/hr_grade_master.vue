<template>
    ...
    <CKFormModal v-model="formModel" :title="params.id ? 'Edit Grade' : 'New Grade'" :endPoint="endPoint"
        :formCode="'hr_grade_master'" :initialData="initialData" :params="params"
...
const title = ref("Grade List");
const endPoint = ref("/hrm/grade_master");
const params = ref({});
...
const columns = ref([
    ...
    { accessorKey: 'grade_name', header: 'Grade Name' },
    { id: 'action' }
]);
const loadData = async () => {
    ...
    try {
        ...
        if (temp.success && Array.isArray(temp.grades)) {
            data.value = temp.grades;
        } else {
...
const filteredData = computed(() => {
    ...
    return data.value.filter(dept =>
        dept.grade_name?.toLowerCase().includes(query)
    );
});
