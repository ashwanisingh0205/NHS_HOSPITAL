<template>
        ...
        <div class="md:w-1/3 lg:w-1/3">
            <CKCardList :loading="loading" :title="title" @handleAdd="handleAdd" v-model="searchQuery">
                <UTable :loading="loading" :data="filteredData" :columns="columns">
                    ...
                    <template #document_type_name-cell="{ row }">
    ...
    <CKFormModal v-model="formModel" :title="params.id ? 'Edit Document Type' : 'New Document Type'" :endPoint="endPoint"
        :formCode="'hr_document_type_master'" :initialData="initialData" :params="params"
...
const title = ref("Document Type List");
const endPoint = ref("/hrm/document_type");
const params = ref({});
...
const columns = ref([
    ...
    { accessorKey: 'document_type_name', header: 'Document Type Name' },
    { id: 'action' }
]);
const loadData = async () => {
    ...
    try {
        ...
        error.value = err.response?.data?.message || err.message || 'Failed to load document types';
    } finally {
        loading.value = false;
    }
};
...
const filteredData = computed(() => {
    ...
    return data.value.filter(dept =>
        dept.document_type_name?.toLowerCase().includes(query)
    );
});
