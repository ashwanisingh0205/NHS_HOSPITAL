<template>
        ...
        <div class="md:w-1/3 lg:w-1/3">
            <CKCardList :loading="loading" :title="title" @handleAdd="handleAdd" v-model="searchQuery">
                <UTable :loading="loading" :data="filteredData" :columns="columns">
                    ...
                    <template #designation_name-cell="{ row }">
    ...
    <CKFormModal v-model="formModel" :title="params.id ? 'Edit Designation' : 'New Designation'" :endPoint="endPoint"
        :formCode="'hr_designation_master'" :initialData="initialData" :params="params"
...
const title = ref("Designation List");
const endPoint = ref("/hrm/designation");
const params = ref({});
...
const columns = ref([
    ...
    { accessorKey: 'designation_name', header: 'Designation Name' },
    { id: 'action' }
]);
const loadData = async () => {
    ...
    try {
        ...
        if (temp.success && Array.isArray(temp.designations)) {
            data.value = temp.designations;
        } else {
            error.value = 'Invalid response format from API';
        }
    } catch (err) {
        error.value = err.response?.data?.message || err.message || 'Failed to load designations';
    } finally {
        loading.value = false;
    }
};
...
const filteredData = computed(() => {
    ...
    return data.value.filter(dept =>
        dept.designation_name?.toLowerCase().includes(query)
    );
});
