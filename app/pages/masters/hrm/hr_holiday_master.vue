        ...
        <div class="md:w-1/3 lg:w-1/3">
            <CKCardList :loading="loading" :title="title" @handleAdd="handleAdd" v-model="searchQuery">
                <UTable :loading="loading" :data="filteredData" :columns="columns">
                    ...
                    <template #holiday_name-cell="{ row }">
...
const title = ref("Holiday List");
const endPoint = ref("/hrm/holiday_master");
const params = ref({});
...
const loadData = async () => {
    ...
    try {
        ...
        if (temp.success && Array.isArray(temp.holidays)) {
            data.value = temp.holidays;
        } else {
            error.value = 'Invalid response format from API';
        }
    } catch (err) {
        error.value = err.response?.data?.message || err.message || 'Failed to load holidays';
    } finally {
        loading.value = false;
    }
};
...
const filteredData = computed(() => {
    ...
    return data.value.filter(dept =>
        dept.holiday_name?.toLowerCase().includes(query)
    );
});
