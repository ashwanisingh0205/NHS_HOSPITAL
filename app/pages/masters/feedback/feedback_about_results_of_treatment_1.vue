...
const title = ref("Feedback about results of treatment - 1");
...
const columns = ref([
    ...
    { accessorKey: 'botd', header: 'Behaviour of doctor' },
    { id: 'action' }
]);
...
const filteredData = computed(() => {
    ...
    return data.value.filter(item =>
        item.botd?.toLowerCase().includes(query) ||
        item.oeot?.toLowerCase().includes(query) ||
        item.swr?.toLowerCase().includes(query)
    );
});
