<template>
    ...
    <CKFormModal v-model="formModel" :title="params.id ? 'Edit Slip Payment' : 'New Slip Payment'" :endPoint="endPoint"
        formCode="slip_payment" :id="id" :params="params" @handleFormSubmit="handleFormSubmit" />
...
const title = ref("Slip Payment List");
const endPoint = ref("/masters/billings/slip_payment");
const params = ref({});
...
const loadData = async () => {
    ...
    try {
        ...
        if (temp.success && Array.isArray(temp.slipPayment)) {
            data.value = temp.slipPayment;
        } else {
            error.value = 'Invalid response format from API';
        }
    } catch (err) {
        error.value = err.response?.data?.message || err.message || 'Failed to load slip payments';
    } finally {
        loading.value = false;
    }
};
