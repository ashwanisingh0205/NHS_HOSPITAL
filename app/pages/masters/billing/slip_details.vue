<template>
    ...
    <CKFormModal v-model="formModel" :title="params.id ? 'Edit Slip Detail' : 'New Slip Detail'" :endPoint="endPoint"
        formCode="slip_details" :id="id" :params="params" @handleFormSubmit="handleFormSubmit" />
...
const title = ref("Slip Details");
const endPoint = ref("/masters/billings/slip_details");
const params = ref({});
...
const columns = ref([
    ...
    { accessorKey: 'service_name', header: 'Service Name' },
    { id: 'action' }
]);
const loadData = async () => {
    ...
    try {
        ...
        if (temp.success && Array.isArray(temp.slipDetails)) {
            data.value = temp.slipDetails;
        } else {
            error.value = 'Invalid response format from API';
        }
    } catch (err) {
        error.value = err.response?.data?.message || err.message || 'Failed to load slip details';
    } finally {
        loading.value = false;
    }
};
