<template>
    ...
    <CKFormModal v-model="formModel" :title="params.id ? 'Edit Financial Year' : 'New Financial Year'"
        :endPoint="endPoint" formCode="financial_year" :id="id" :params="params"
...
const title = ref("Financial Years List");
const endPoint = ref("/masters/accounts/financial_year_master");
const params = ref({});
...
const loadData = async () => {
    ...
    try {
        ...
        if (temp.success && Array.isArray(temp.years)) {
            data.value = temp.years;
        } else {
            error.value = 'Invalid response format from API';
        }
    } catch (err) {
        error.value = err.response?.data?.message || err.message || 'Failed to load financial years';
    } finally {
        loading.value = false;
    }
};
