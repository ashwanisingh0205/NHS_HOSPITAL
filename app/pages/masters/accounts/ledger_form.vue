<template>
    ...
    <CKFormModal v-model="formModel" :title="params.id ? 'Edit Ledger' : 'New Ledger'" :endPoint="endPoint"
        formCode="ledger_master" :id="id" :params="params" @handleFormSubmit="handleFormSubmit" />
...
const title = ref("Ledger List");
const endPoint = ref("/masters/accounts/ledger_master");
const params = ref({});
...
const loadData = async () => {
    ...
    try {
        ...
        if (temp.success && Array.isArray(temp.ledgers)) {
            data.value = temp.ledgers;
        } else {
            error.value = 'Invalid response format from API';
        }
    } catch (err) {
        error.value = err.response?.data?.message || err.message || 'Failed to load ledgers';
    } finally {
        loading.value = false;
    }
};
