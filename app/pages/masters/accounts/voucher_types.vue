<template>
    ...
    <CKFormModal v-model="formModel" :title="params.id ? 'Edit Voucher Type' : 'New Voucher Type'" :endPoint="endPoint"
        formCode="voucher_type" :id="id" :params="params" @handleFormSubmit="handleFormSubmit" />
...
const title = ref("Voucher Type List");
const endPoint = ref("/masters/accounts/voucher_type");
const params = ref({});
...
const columns = ref([
    ...
    { accessorKey: 'name', header: 'Voucher Type Name' },
    { id: 'action' }
]);
const loadData = async () => {
    ...
    try {
        ...
        if (temp.success && Array.isArray(temp.types)) {
            data.value = temp.types;
        } else {
