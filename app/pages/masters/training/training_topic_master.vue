<template>
        ...
        <div class="md:w-1/3 lg:w-1/3">
            <CKCardList :loading="loading" :title="title" @handleAdd="handleAdd" v-model="searchQuery">
                <UTable :loading="loading" :data="filteredData" :columns="columns">
                    ...
                    <template #training_topic_name-cell="{ row }">
                        {{ row.original.training_topic_name }}
                    </template>
    ...
    <CKFormModal v-model="formModel" :title="params.id ? 'Edit Training Topic' : 'New Training Topic'" :endPoint="endPoint"
        :id="id" :params="params"
...
const title = ref("Training Topic List");
const endPoint = ref("/training/topic");
const params = ref({});
...
const columns = ref([
    ...
    { accessorKey: 'training_topic_name', header: 'Training Topic Name' },
    { id: 'action' }
]);
const loadData = async () => {
    ...
    try {
        ...
        error.value = err.response?.data?.message || err.message || 'Failed to load training topics';
    } finally {
        loading.value = false;
    }
};
...
const filteredData = computed(() => {
    ...
    return data.value.filter(topic =>
        topic.training_topic_name?.toLowerCase().includes(query)
    );
});
