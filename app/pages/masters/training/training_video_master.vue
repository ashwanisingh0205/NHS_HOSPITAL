<template>
    ...
    <CKFormModal v-model="formModel" :title="params.id ? 'Edit Training Video' : 'New Training Video'" :endPoint="endPoint"
        :id="id" :params="params"
...
const title = ref("Training Video List");
const endPoint = ref("/training/video");
const params = ref({});
...
const columns = ref([
    ...
    { accessorKey: 'video_title', header: 'Video Title' },
    { id: 'action' }
]);
const loadData = async () => {
    ...
    try {
        ...
        error.value = err.response?.data?.message || err.message || 'Failed to load training videos';
    } finally {
        loading.value = false;
    }
};
