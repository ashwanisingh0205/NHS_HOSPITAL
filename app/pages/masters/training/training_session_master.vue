<template>
    ...
    <CKFormModal v-model="formModel" :title="params.id ? 'Edit Training Session' : 'New Training Session'" :endPoint="endPoint"
        :id="id" :params="params"
...
const title = ref("Training Session Master List");
const endPoint = ref("/training/session");
const params = ref({});
...
const loadData = async () => {
    ...
    try {
        ...
        error.value = err.response?.data?.message || err.message || 'Failed to load training sessions';
    } finally {
        loading.value = false;
    }
};
