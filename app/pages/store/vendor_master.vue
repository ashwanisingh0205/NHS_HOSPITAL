<template>
    ...
    <CKFormModal v-model="formModel" :title="params.id ? 'Edit Vendor Master' : 'New Vendor Master'"
        :endPoint="endPoint" formCode="store_vendor_master" :id="id" :params="params"
...
const title = ref("Store Vendor Master");
...
const loadData = async () => {
    ...
    try {
        const response = await $axios.get(endPoint.value, {
            params: { form_code: 'store_vendor_master' }
        });
        ...
        error.value = err.response?.data?.message || err.message || 'Failed to load vendors';
    } finally {
        loading.value = false;
    }
};
...
/* ------------------ Add Button ------------------ */
const handleAdd = () => {
    params.value = { form_code: 'store_vendor_master' };
    formModel.value = true;
};

/* ------------------ Edit Button ------------------ */
const handleEdit = async (item) => {
    params.value = { id: item.original.id, form_code: 'store_vendor_master' };
