    ...
    <div class=" space-y-4 bg-white dark:bg-gray-950 min-h-screen">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <ServiceBreakdownCard
                ...
                :progress-color="service.progressColor"
                @add-payment="handleAddPayment(service.title)"
                @add-service="handleAddService(service.title)"
            />
        </div>

        <!-- Payment Trend & Summary Section -->
        ...
        <!-- Bill Actions & Date Selector -->
        <BillActionsCard
            :week-days="weekDays"
            :selected-date="selectedDate"
            @date-select="selectedDate = $event"
            @add-service="handleAddService('General')"
            @add-payment="handleAddPayment('General')"
            @add-discount="handleAddDiscount"
        />

        <!-- Payor Information -->
        <PayorInfoCard :payor="payorInfo" @view="handleViewPayor" />
            ...
            <div class="mt-4">
                ...
                >
                    ...
                    <template #quantity-cell="{ row }">
                        <span class="text-center">{{ row.original.quantity }}</span>
                    </template>
                    <template #rate-cell="{ row }">
                        <span class="text-right">₹{{ formatNumber(row.original.rate) }}</span>
                    </template>
                    <template #total-cell="{ row }">
                        <span class="text-right font-medium">₹{{ formatNumber(row.original.total) }}</span>
                    </template>
                </UTable>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <div class="lg:col-span-2">
                <UCard>
                    ...
                    <div class="space-y-3">
                        <UCard
                            v-for="payment in hospitalPayments"
                            :key="payment.id"
...
const totalBill = computed(() => {
    return services.value.reduce((sum, service) => sum + service.totalAmount, 0)
})

const totalPayment = computed(() => {
    return services.value.reduce((sum, service) => sum + service.paidAmount, 0)
})
...
const currentTabTotal = computed(() => {
    return billDetails.value.reduce((sum, item) => sum + item.total, 0)
})
...
const handleAddPayment = (service) => {
    console.log('Add payment for:', service)
    // Implement payment logic
}

const handleAddService = (service) => {
    console.log('Add service for:', service)
...
const handleViewPayor = () => {
    console.log('View payor details')
    // Implement view payor logic
}
